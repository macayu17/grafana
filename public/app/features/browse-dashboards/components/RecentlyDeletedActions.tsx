...
export function RecentlyDeletedActions() {
  ...
  const selectedDashboardOrigin = useMemo(() => {
    ...
    for (const selectedDashboard of selectedDashboards) {
      ...
      // Searcher changes the location from empty string to 'general' / 'root' for items
      // with no parent, but the restore API doesn't work with those folder UIDs — convert
      // back to an empty string so the dashboard is restored to the root.
  ...
  const onRestore = async (restoreTarget: string) => {
    ...
    for (const uid of selectedDashboards) {
      ...
      // Search API returns items with no parent with a location of 'general' / 'root',
      // so we need to convert that back to undefined.
