...
func TestGetNamespaceByTitle(t *testing.T) {
	...
	_, err := store.GetNamespaceByTitle(context.Background(), "Test Folder", 1, nil, folder.LegacyRootFolderUID)
	require.Error(t, err)
...
func TestGetOrCreateNamespaceByTitle(t *testing.T) {
	store := DBstore{}
	_, created, err := store.GetOrCreateNamespaceByTitle(context.Background(), "", 1, nil, folder.LegacyRootFolderUID)
