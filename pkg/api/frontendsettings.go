...
func (hs *HTTPServer) getFrontendSettings(c *contextmodel.ReqContext) (*dtos.FrontendSettingsDTO, error) {
	...
	// Start with all static, cfg-derived settings.
	s, err := hs.buildFrontendSettings(c)
	if err != nil {
		return nil, err
	}
...
// buildFrontendSettings constructs the full FrontendSettingsDTO by combining the
// static settings from GetFrontendSettings with the dynamic, per-request parts
// that require access to runtime services (plugins, datasources, OAuth, etc.).
//
//nolint:gocyclo
func (hs *HTTPServer) buildFrontendSettings(c *contextmodel.ReqContext) (*dtos.FrontendSettingsDTO, error) {
	...
	if hs.Cfg.Anonymous.HideVersion && !c.IsSignedIn {
		feSettings.BuildInfo.HideVersion = true
		feSettings.BuildInfo.Version = ""
		feSettings.BuildInfo.VersionString = setting.ApplicationName
		feSettings.BuildInfo.Commit = ""
		feSettings.BuildInfo.CommitShort = ""
		feSettings.BuildInfo.Buildstamp = 0
	}

	// Update checker — only available in the full server.
	feSettings.BuildInfo.LatestVersion = hs.grafanaUpdateChecker.LatestVersion()
	feSettings.BuildInfo.HasUpdate = hs.grafanaUpdateChecker.UpdateAvailable()

	// License URL requires access control context.
	hasAccess := accesscontrol.HasAccess(hs.AccessControl, c)
	feSettings.LicenseInfo.LicenseUrl = hs.License.LicenseURL(hasAccess(licensing.PageAccess))

	// Per-org plugins, datasources, panels.
	...
	for _, ap := range availablePlugins[plugins.TypeApp] {
		apps[ap.Plugin.ID] = hs.newAppDTO(c.Req.Context(), ap.Plugin, ap.Settings)
	}
	...
	feSettings.DefaultDatasource = defaultDS
	feSettings.Datasources = dataSources
	feSettings.Panels = panels
	feSettings.Apps = apps
	feSettings.AllowOrgCreate = (hs.Cfg.AllowUserOrgCreate && c.IsSignedIn) || c.IsGrafanaAdmin
	feSettings.PublicDashboardAccessToken = c.PublicDashboardAccessToken

	// Runtime services.
	feSettings.RendererAvailable = hs.RenderService.IsAvailable(c.Req.Context())
	feSettings.RendererVersion = hs.RenderService.Version()
	feSettings.PluginCatalogManagedPlugins = hs.managedPluginsService.ManagedPlugins(c.Req.Context())

	// OAuth and SAML providers.
	feSettings.Oauth = hs.getEnabledOAuthProviders()
	feSettings.SamlEnabled = hs.samlEnabled()
	feSettings.SamlName = hs.samlName()

	// Per-provider OAuth skip-org-role-sync flags.
	...
	feSettings.Auth.GoogleSkipOrgRoleSync = parseSkipOrgRoleSyncEnabled(oauthProviders[social.GoogleProviderName])
	feSettings.Auth.GrafanaComSkipOrgRoleSync = parseSkipOrgRoleSyncEnabled(oauthProviders[social.GrafanaComProviderName])
	feSettings.Auth.GenericOAuthSkipOrgRoleSync = parseSkipOrgRoleSyncEnabled(oauthProviders[social.GenericOAuthProviderName])
	feSettings.Auth.AzureADSkipOrgRoleSync = parseSkipOrgRoleSyncEnabled(oauthProviders[social.AzureADProviderName])
	feSettings.Auth.GithubSkipOrgRoleSync = parseSkipOrgRoleSyncEnabled(oauthProviders[social.GitHubProviderName])
	feSettings.Auth.GitLabSkipOrgRoleSync = parseSkipOrgRoleSyncEnabled(oauthProviders[social.GitlabProviderName])
	feSettings.Auth.OktaSkipOrgRoleSync = parseSkipOrgRoleSyncEnabled(oauthProviders[social.OktaProviderName])
	...
	if hs.pluginsCDNService != nil && hs.pluginsCDNService.IsEnabled() {
		...
		feSettings.PluginsCDNBaseURL = cdnBaseURL
	}

	// Per-org namespace and scope endpoints.
	feSettings.Namespace = hs.namespacer(c.OrgID)

	//nolint:staticcheck // not yet migrated to OpenFeature
	if hs.Features.IsEnabled(c.Req.Context(), featuremgmt.FlagScopeFilters) {
		feSettings.ListScopesEndpoint = hs.Cfg.ScopesListScopesURL
		feSettings.ListDashboardScopesEndpoint = hs.Cfg.ScopesListDashboardsURL
	}

	return feSettings, nil
}
